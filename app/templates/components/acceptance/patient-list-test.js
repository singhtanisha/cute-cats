...
moduleForAcceptanceAuth('Acceptance - Visual - Core - Charting | Patient list - v2', {
    beforeEach() {
...
function validateFirstScheduledPatientColumn(assert, column, expected) {
    assert.equal(getText($(`#appointments .row-patient ${de(column)}`).first()), expected, `The first patient's ${column} is correct`);
}
